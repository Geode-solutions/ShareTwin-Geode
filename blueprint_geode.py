...
def convert_file():
    try:
        ...
        native_file_path = os.path.join(
            UPLOAD_FOLDER, id + '.' + native_extension)

        saved_viewable_file_path = functions.geode_objects.objects_list(
        )[object_type]['save_viewable'](data, viewable_file_path)
        print('Saved viewable', flush=True)
        functions.geode_objects.objects_list(
        )[object_type]['save'](data, native_file_path)
...
def asign_geographic_coordinate_system():
    ...
    if id is None:
        return flask.make_response({'name': 'Bad Request', 'description': 'No id sent'}, 400)
    if crs_authority is None:
        return flask.make_response({'name': 'Bad Request', 'description': 'No crs_authority sent'}, 400)
    if crs_code is None:
        return flask.make_response({'name': 'Bad Request', 'description': 'No crs_code sent'}, 400)
    if crs_name is None:
        return flask.make_response({'name': 'Bad Request', 'description': 'No crs_name sent'}, 400)
